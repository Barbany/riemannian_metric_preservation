...
def riemannian_metric(model, inputs):
    ...
    jac = ag.functional.jacobian(_func_sum, inputs, create_graph=True, vectorize=True)
...
class DiffGeomProps(nn.Module):
    ...
    def __init__(self, normals=True, curv_mean=True, curv_gauss=True, fff=False, gpu=True):
    ...
    def forward(self, X, uv):
        ...
        if not (self._comp_normals or self._comp_cmean or self._comp_cgauss or self._comp_fff):
            return ret
        ...
        if self._comp_normals:
            if D == 2:
                raise Exception("Normals can only be comupted for " "2D -> 3D mapping.")
            ret["normals"] = normals
        ...
        if self._comp_cmean or self._comp_cgauss:
            if D == 2:
                raise Exception("Curvature can only be computed for " "2D -> 3D mapping")
        ...
        if self._comp_cmean:
            ...
            assert gE is not None and gG is not None
            cmean = (
                torch.sum(
                    (-normals / detg[..., None])
                    * (
    ...
    def df(self, x, wrt):
        ...
        return ag.grad(
            x.flatten(),
            wrt,
            grad_outputs=torch.ones(B * M, dtype=torch.float32).to(x.device),
            create_graph=True,
        )[0]
